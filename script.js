...
// ────────────────────────────────────────────────────────────────
// REAL AUTH CONFIG (Firebase)
// ────────────────────────────────────────────────────────────────
// To make login/signup/Google sign‑in fully real:
// 1. Go to https://console.firebase.google.com, create a project.
// 2. Add a Web app, enable Email/Password and Google providers in Authentication.
// 3. Copy your config below (apiKey, authDomain, etc).
// Until you fill this, the app will stay in DEMO auth mode.
let auth = null;
...
if (window.firebase && firebaseConfig.apiKey !== "YOUR_API_KEY_HERE") {
  try {
    firebase.initializeApp(firebaseConfig);
    auth = firebase.auth();
    auth.useDeviceLanguage();
  } catch (e) {
    console.error("Firebase init failed, staying in demo auth mode:", e);
    auth = null;
  }
}

// Map our symbols to CoinGecko IDs for live prices
...
async function fetchLivePricesOnce() {
  const ids = Object.values(LIVE_SYMBOL_MAP).join(',');
  ...
  try {
    ...
    // Update our COINS array in-place so all calculations use live prices
    data.forEach(m => {
      const sym = (m.symbol || '').toUpperCase();
      const id  = m.id;
      const coin = COINS.find(c =>
        LIVE_SYMBOL_MAP[c.sym] === id ||
        c.sym.toUpperCase() === sym
      );
      if (!coin) return;

      coin.price  = m.current_price;
      coin.change = typeof m.price_change_percentage_24h === 'number'
        ? m.price_change_percentage_24h
        : coin.change;
      coin.vol    = '$' + (m.total_volume || 0).toLocaleString();
      coin.mcap   = '$' + (m.market_cap || 0).toLocaleString();
    });

    // Re-render all price‑dependent UI
    buildTicker();
    renderMarkets();
...
function startLivePricePolling() {
  ...
  livePriceTimer = setInterval(fetchLivePricesOnce, 60000);
}
...
function refreshData() {
  ...
  renderChart(chartData);
  renderOrderBook(COINS[0].price);
  // Force a fresh live price fetch and then re-render
  fetchLivePricesOnce().then(() => {
    renderMarkets();
    renderPortfolio();
  });
}
...
async function doLogin() {
  ...
  try {
    ...
    document.getElementById('success-title').textContent = `Welcome back, ${email.split('@')[0]}!`;
    ...
    if (err.message === 'DEMO_BAD_CREDENTIALS') {
      msg = 'Incorrect email or password. Try demo@nexus.io / Demo@1234';
    }
    showAuthError('login-error', 'login-error-msg', msg);
...
async function doSignup() {
  ...
  try {
    ...
    if (err.code === 'auth/weak-password') msg = 'Password is too weak. Please strengthen it.';
    showAuthError('signup-error', 'signup-error-msg', msg);
  }
}
...
async function doSocialLogin(provider) {
  if (provider === 'Google' && auth) {
    try {
      ...
      showScreen('screen-success');
      document.getElementById('success-title').textContent = 'Google Connected';
      document.getElementById('success-msg').textContent = 'IDENTITY VERIFIED. ENTERING NEXUS DASHBOARD.';
      setTimeout(enterApp, 1200);
      return;
    } catch (err) {
      console.error('Google sign-in failed:', err);
      showAuthError('login-error', 'login-error-msg', 'Google sign‑in failed. Check console for details.');
      return;
    }
  }

  // Fallback demo behavior for other providers or when Firebase not configured
  showScreen('screen-success');
...
async function sendResetCode() {
  ...
  if (auth) {
    // Real password reset email via Firebase
    try {
      await auth.sendPasswordResetEmail(email);
      alert('Password reset email sent. Check your inbox.');
    } catch (err) {
      console.error('Reset email error:', err);
      alert('Could not send reset email. Check console for details.');
    }
    return;
  }

  // Demo OTP flow (no real email delivery)
  resetEmail = email;
  generatedOTP = generateOTPCode();
  showScreen('screen-otp');
  document.getElementById('otp-subtitle').textContent = `CODE SHOWN BELOW FOR DEMO (${email.toUpperCase()})`;
...
// ── Enter the app ──
// Kept for compatibility, but main entry is now automatic on page load.
function enterApp() {
...
// (Random demo price jitter removed so live web prices stay accurate)
...
// Init pages + dashboard automatically on load so the app stays visible
document.addEventListener('DOMContentLoaded', () => {
  // On first load we stay on the login screen.
  // Just prep background systems; real entry is via enterApp() after auth.
  updateConverterRate();
  renderWatchlist();
  renderNews();
});
